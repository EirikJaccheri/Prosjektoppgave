...
\begin{equation}
    S^{e-e}_{eff} = -\sum_{q,s,k,k',\sigma,\sigma'} \frac{1}{2 \beta} V_{k k' q s}
      C^{\dagger}_{k' + q,\sigma'}C_{k',\sigma'} C^{\dagger}_{k - q,\sigma}C_{k,\sigma}\;,\nonumber \\
    \label{effective hamiltonian}
\end{equation}
with
\begin{equation}
V_{k k' q s} = 2  \frac{g_{\textbf{k'},s}^{(\textbf{q})} g_{\textbf{k},s}^{(-\textbf{q})}  }{N\left(-i\hbar\Omega_n + \hbar \omega_{\textbf{q}}\right)}\;.
\end{equation}
...
\todo{fikse beta V faktorer, introdusere eksplisitt definisjon av V, rydde opp i indekser og summer }
...
\begin{equation}
    Z = \int D(C^{\dagger}, C) e^{-\frac{1}{2\beta}C^{\dagger}_{\alpha} C^{\dagger}_{\alpha'} V_{\alpha \alpha' \beta \beta'} C_{\beta} C_{\beta'}} \;,
...
\begin{equation} 
    1 = \int D(\bar{\Delta},\Delta) e^{\beta \bar{\Delta}_{\alpha \alpha'} V^{-1}_{\alpha \alpha' \beta \beta'} \Delta_{\beta \beta'} }\;,
\end{equation}
...
Using the HS-transformation~\cref{hs-transformation}, and introducing the new coordinates $Q = (k + k') / 2 $, $p = p' + q$ and $p' = (k - k')/2$ we can rewrite the effective action in~\cref{effective action}
\begin{align}
 - \hbar S_{eff}[C^{\dagger},C] &= \sum_{k,\sigma} \epsilon_k C^{\dagger}_{k,\sigma} C_{k,\sigma} - \frac{1}{2\beta} \sum_{Q,s,p,p',\sigma,\sigma'} C^{\dagger}_{Q + p,\sigma'} C^{\dagger}_{Q - p,\sigma} V_{Q s p p'}  C_{Q + p',\sigma'} C_{Q - p',\sigma}\label{effective action transformed}\\
                       %
                       &=  \sum_{k,\sigma} \epsilon_k C^{\dagger}_{k,\sigma} C_{k,\sigma} +  \sum_{Q,s,p,p',\sigma,\sigma'} \beta \bar{\Delta}^{Q}_{\sigma' \sigma}(p) V^{-1}_{Q s p p'} \Delta^{Q}_{\sigma' \sigma}(p')\nonumber\\
                       %
                       &- \sum_{Q}\left( C^{\dagger}_{Q + p,\sigma'} C^{\dagger}_{Q - p,\sigma} \Delta^{Q}_{\sigma' \sigma}(p)+ \bar{\Delta}^{Q}_{\sigma' \sigma}(p)  C_{Q + p,\sigma'} C_{Q - p,\sigma}\right)\nonumber\\
                       %
                       &:= \sum_{Q,s,p,p',\sigma,\sigma'} \beta \bar{\Delta}^{Q}_{\sigma' \sigma}(p) V^{-1}_{Q s p p'} \Delta^{Q}_{\sigma' \sigma}(p') + \frac{1}{2} \sum_{Q p} \bar{\Psi}_{Q p} G^{-1}_ {Q p} \Psi_{Q p}\;,
\end{align}
with 
\begin{equation}
     G_{Q p}^{-1} = 
...
\end{bmatrix}\;.
\end{equation} 
At this point it is necessary to make the approximation that $Q$ has a fixed value, so that we can remove the sum over $Q$ in~\cref{effective action transformed}. The value of $Q$ is chosen so that it gives the highest possible critical temperature~($T_c$), making it responsible for the phase transition. After making the choice of $Q$, all the $\Psi_{Q p}$-vectors are independent, and we can perform the Gaussian integrals 
\begin{align}
    \int D(C^{\dagger},C) e^{\frac{1}{2} \sum_{p} \bar{\Psi}_{p} G^{-1}_ {p} \Psi_{p}} &= \Pi_p \det{G^{-1}(p)} \nonumber\\
                                                                                               &= e^{\sum_p \Tr{\ln{G^{-1}(p)}}} \;\nonumber,
\end{align}
where we have used the identity $\ln{\det{A}} = \Tr{\ln{A}}$. The path integral~\cref{effective partion function} then becomes
\begin{equation}
    Z_{eff} = \int D(\bar{\Delta}, \Delta) e^{\left(\beta\sum_{s,p,p',\sigma,\sigma'} \bar{\Delta}_{\sigma' \sigma}(p) V^{-1}_{s p p'} \Delta_{\sigma' \sigma}(p') + \sum_p \Tr{\ln{G^{-1}(p)}}\right) / \hbar}\;.
\end{equation}
...
\todo{tror sluttresultat er rett, men tydeligjør fremgang}
\begin{align}
    \frac{\delta S_{eff}}{\delta \Delta_{\sigma_1,\sigma_2}^{p}} &= 0 \\
    \beta\sum_{s,p'} \bar{\Delta}_{\sigma_1 \sigma_2}(p') V^{-1}_{s p' p} + \sum_{p'} \Tr{G(p') 
    \begin{bmatrix}
...
%}
$\Tr{G(p) B(\sigma_1 = \uparrow \uparrow)} = $
\begin{equation}
\frac{\Delta_{dd} \bar{\Delta}_{dd} \bar{\Delta}_{uu} - \Delta_{dd} \bar{\Delta}_{du} \bar{\Delta}_{ud} + \bar{\Delta}_{uu} \epsilon_{Q-p}^{2}}{\Delta_{dd} \Delta_{uu} \bar{\Delta}_{dd} \bar{\Delta}_{uu} - \Delta_{dd} \Delta_{uu} \bar{\Delta}_{du} \bar{\Delta}_{ud} + \Delta_{dd} \bar{\Delta}_{dd} \epsilon_{Q+p} \epsilon_{Q-p} - \Delta_{du} \Delta_{ud} \bar{\Delta}_{dd} \bar{\Delta}_{uu} + \Delta_{du} \Delta_{ud} \bar{\Delta}_{du} \bar{\Delta}_{ud} + \Delta_{du} \bar{\Delta}_{ud} \epsilon_{Q+p} \epsilon_{Q-p} + \Delta_{ud} \bar{\Delta}_{du} \epsilon_{Q-p}^{2} + \Delta_{uu} \bar{\Delta}_{uu} \epsilon_{Q-p}^{2} + \epsilon_{Q+p} \epsilon_{Q-p}^{3}}
\end{equation}
$\Tr{G(p) B(\sigma_1 = \uparrow \downarrow)} = $
\begin{equation}
\frac{- \Delta_{du} \bar{\Delta}_{dd} \bar{\Delta}_{uu} + \Delta_{du} \bar{\Delta}_{du} \bar{\Delta}_{ud} + \bar{\Delta}_{du} \epsilon_{Q-p}^{2}}{\Delta_{dd} \Delta_{uu} \bar{\Delta}_{dd} \bar{\Delta}_{uu} - \Delta_{dd} \Delta_{uu} \bar{\Delta}_{du} \bar{\Delta}_{ud} + \Delta_{dd} \bar{\Delta}_{dd} \epsilon_{Q+p} \epsilon_{Q-p} - \Delta_{du} \Delta_{ud} \bar{\Delta}_{dd} \bar{\Delta}_{uu} + \Delta_{du} \Delta_{ud} \bar{\Delta}_{du} \bar{\Delta}_{ud} + \Delta_{du} \bar{\Delta}_{ud} \epsilon_{Q+p} \epsilon_{Q-p} + \Delta_{ud} \bar{\Delta}_{du} \epsilon_{Q-p}^{2} + \Delta_{uu} \bar{\Delta}_{uu} \epsilon_{Q-p}^{2} + \epsilon_{Q+p} \epsilon_{Q-p}^{3}}
\end{equation}
